...
\subsection{Diagnóstico (Situación Actual): Desalineación Institucional}
Bajo el marco institucional vigente, se ha observado una fragmentación operativa derivada de la interpretación divergente de atribuciones entre la autoridad encargada de la Política Pública (SENER) y el órgano regulador (CRE).

Esta falta de sincronización ha ocasionado que la emisión de regulaciones técnicas necesarias para la transición energética se retrase o disocie de las metas establecidas en el Programa de Desarrollo del Sistema Eléctrico Nacional (PRODESEN), generando vacíos normativos y una excesiva judicialización del sector debido a la incertidumbre regulatoria.

\subsection{Visión Objetivo (Situación Objetivo): Alineación Estratégica y Rectoría}
Se implementa un modelo de gobernanza basado en la Unidad de Propósito, reafirmando la Rectoría del Estado en materia energética.

\begin{itemize}
    \item \textbf{SENER (Definición Estratégica):} Establece la Política Energética, las metas de participación de energías limpias y la Planeación Vinculante (El ``Qué'' y el ``Cuándo'').
    \item \textbf{CNE/CRE (Instrumentación Técnica):} Diseña y opera los mecanismos regulatorios, sistemas de información (SNIEr) y esquemas de vigilancia necesarios para materializar dichas metas con eficiencia operativa (El ``Cómo'').
\end{itemize}

La regulación técnica deja de ser un fin en sí mismo para convertirse en el instrumento habilitador de la política pública.

\subsection{Arquitectura de Sistemas (Control de Gestión)}
El SNIEr reflejará esta gobernanza mediante un esquema de Control de Acceso Basado en Roles (RBAC):

\begin{itemize}
    \item \textbf{Nivel de Política (SENER):} Acceso exclusivo para la configuración de ``Variables Maestras'' del sistema (Metas Anuales de CELs, Factores de Emisión, Criterios de Tecnología Limpia).
    \item \textbf{Nivel Operativo (CNE/CRE):} Acceso para la ejecución de procesos (Emisión, Validación, Sanción) sin privilegios para alterar las definiciones de política preestablecidas en el código, garantizando la integridad de las metas nacionales.
\end{itemize}

\subsection{Reingeniería de Procesos (Ciclo Regulatorio)}
...
\begin{tabladoradoCorto}
  ...
  \begin{tabularx}{\textwidth}{L{0.20\textwidth} L{0.35\textwidth} X}
    ...
    \midrule
    \textbf{Definición de Metas} & SENER publicaba avisos que requerían una adopción discrecional o diferida por parte del Regulador. & \textbf{Vinculación Automática:} La publicación de metas en el PRODESEN actualiza automáticamente los parámetros de cumplimiento en el SNIEr, detonando las obligaciones de mercado. \\
    \textbf{Resolución de Conflictos} & Mesas de negociación interinstitucional prolongadas o controversias constitucionales. & \textbf{Criterio Unificado:} Se establece la preeminencia de la Política Pública en la definición de objetivos de mercado, reservando la autonomía técnica para la metodología de implementación. \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}

\subsection{Blindaje Normativo (Reforma Legal Sugerida)}
Se propone fortalecer el mecanismo de coordinación mediante una adición a la Ley de los Órganos Reguladores Coordinados en Materia Energética (LORCME).

\begin{displayquote}
\textbf{Adición del Artículo 22 Bis a la LORCME:}

\textbf{Artículo 22 Bis.} Los Órganos Reguladores Coordinados, en el ámbito de su competencia, deberán asegurar que la emisión de regulación, metodologías y actos administrativos fomente y habilite el cumplimiento de los objetivos establecidos en la Política Energética y en el Programa de Desarrollo del Sistema Eléctrico Nacional.

\textbf{II.} Las normas y disposiciones técnicas que se expidan no podrán contravenir, obstaculizar o hacer nugatorias las metas de transición energética y seguridad nacional definidas por la Secretaría, debiendo prevalecer el principio de coordinación y unidad de propósito del Estado.
\end{displayquote}

\subsection{Beneficios e Impacto}
\begin{itemize}
    \item \textbf{Certeza Jurídica:} Elimina la ``doble señal'' al mercado; los inversionistas reciben directrices coherentes alineadas desde la política hasta la regulación operativa.
    \item \textbf{Agilidad de Implementación:} Reduce drásticamente el tiempo de adopción de nuevas políticas de transición (Time-to-Market regulatorio).
    \item \textbf{Reducción de Litigios:} Al clarificar la jerarquía funcional, se minimizan los espacios para impugnaciones basadas en conflictos competenciales entre autoridades.
\end{itemize}

\portadaseccion{15}{Proceso de Inscripción}{}
\section{Proceso de Inscripción}

\subsection{Diagnóstico (Situación Actual): Duplicidad de Requisitos de Inscripción}
El procedimiento vigente de registro en el Sistema de Gestión de Certificados (S-CEL) opera bajo un esquema de solicitud rogada que funciona de manera desconectada del otorgamiento de Permisos de Generación. Esto obliga a los Participantes a presentar nuevamente documentación legal y técnica (como Actas Constitutivas y Diagramas Unifilares) que ya fue validada previamente por la Comisión durante la etapa de permisos.

Esta duplicidad administrativa contraviene los principios de simplificación y mejora regulatoria, generando tiempos de espera (latencia administrativa) que impiden la emisión de Certificados durante los primeros meses de operación de la Central, afectando el flujo de ingresos del proyecto.

\subsection{Visión Objetivo (Situación Objetivo): Inscripción de Oficio e Interoperabilidad}
Se implementa el principio de Captura Única, transformando el registro en un trámite de oficio (``Zero-Touch Enrollment'').

\begin{itemize}
    \item \textbf{Trigger Operativo:} La declaratoria de entrada en operación comercial emitida por el CENACE detona automáticamente el alta del Participante en el SNIEr.
    \item \textbf{Expediente Digital Único:} El sistema hereda la identidad legal y técnica de la Central directamente de la base de datos de Permisos de la CNE, eliminando la necesidad de volver a cargar documentos.
    \item \textbf{Validación:} El Participante solo ingresa al sistema para validar sus datos precargados y firmar los términos de uso mediante su firma electrónica (e.firma).
\end{itemize}

\subsection{Arquitectura de Sistemas (Flujo de Datos)}
El SNIEr funcionará como un repositorio centralizado de identidad digital interoperable:

\begin{itemize}
    \item \textbf{Sincronización:} Conexión vía API con el Registro Público de la Comisión (Permisos) para extraer la ``Identidad Estática'' (Dueño, Tecnología, Ubicación).
    \item \textbf{Activación:} Webhook desde el CENACE que notifica el cambio de estatus a ``Operativo'' e inyecta la ``Identidad Dinámica'' (Clave de Mercado, Nodo de Precio).
    \item \textbf{Notificación:} El sistema envía automáticamente las credenciales de acceso al correo registrado del Representante Legal.
\end{itemize}

\subsection{Reingeniería de Procesos (Simplificación)}

\begin{tabladoradoCorto}
  \caption{Tabla 15.1 Del Trámite Manual al Proceso Automatizado}
  \begin{tabularx}{\textwidth}{L{0.20\textwidth} L{0.35\textwidth} X}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Etapa} & \encabezadodorado{Modelo Anterior (Manual)} & \encabezadodorado{Nuevo Modelo (Automatizado)} \\
    \midrule
    \textbf{Inicio} & El usuario integraba un expediente digital y lo cargaba manualmente en la OPE (Oficialía de Partes Electrónica). & \textbf{Proactivo:} El sistema inicia el proceso al detectar la actividad comercial en el mercado mayorista. \\
    \textbf{Validación} & Revisión humana secuencial de documentos legales y técnicos. & \textbf{Algorítmica:} Validación automática de consistencia de datos (RFC Permiso = RFC Contrato Interconexión). \\
    \textbf{Tiempo de Respuesta} & Promedio de meses (sujeto a carga de trabajo y prevenciones). & \textbf{Inmediato:} Habilitación de la cuenta en un plazo máximo de 5 días hábiles tras el inicio de operaciones. \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}

\subsection{Blindaje Normativo (Reforma a las DACG)}
Se propone la modificación del Numeral 8 de las Disposiciones Administrativas (RES/174/2016) para eliminar el requisito de solicitud por escrito.

\begin{displayquote}
\textbf{Modificación al Numeral 8 de las Disposiciones (RES/174/2016):}

\textbf{Numeral 8. De la Inscripción Simplificada y de Oficio.}
\begin{enumerate}[label=\Roman*.]
    \item La inscripción de Centrales Eléctricas Limpias y Centros de Carga en el Sistema se realizará de oficio por parte de la Comisión, tomando como base la información contenida en el Título de Permiso y la notificación de inicio de operaciones comerciales emitida por el CENACE.
    \item Los Participantes del Mercado no estarán obligados a presentar documentación adicional para su registro, salvo aquella estrictamente necesaria para acreditar atributos específicos no contenidos en los registros institucionales previos.
    \item La Comisión notificará al Participante la habilitación de su cuenta en el Sistema en un plazo no mayor a 5 días hábiles posteriores a la recepción de la notificación del CENACE. El Participante deberá ingresar al Sistema para validar la información precargada y suscribir los términos y condiciones de uso mediante su firma electrónica avanzada.
\end{enumerate}
\end{displayquote}

\subsection{Beneficios e Impacto}
\begin{itemize}
    \item \textbf{Eficiencia Económica:} Permite la monetización de los CELs desde el primer mes de operación, eliminando el lucro cesante por trámites burocráticos.
    \item \textbf{Integridad de Datos:} Reduce el error humano en la captura de información al utilizar los datos maestros del Permiso y del Contrato de Interconexión.
    \item \textbf{Transparencia:} Elimina la discrecionalidad en los tiempos de respuesta y cierra espacios a prácticas irregulares para la agilización de trámites.
\end{itemize}

\portadaseccion{16}{CEL y Factor Emisiones}{}
\section{CEL y Factor Emisiones}

\subsection{Diagnóstico (Situación Actual): Desvinculación del Valor Ambiental}
En su concepción original, el Certificado de Energías Limpias (CEL) fue diseñado estrictamente como un instrumento para acreditar la producción de energía eléctrica libre de combustibles fósiles (1 CEL = 1 MWh). Sin embargo, el instrumento carece actualmente de un atributo de datos que vincule esta generación con su impacto climático real, es decir, con las toneladas de CO$_2$ equivalente (tCO$_2$e) desplazadas.

Esta ``ceguera de carbono'' obliga a los usuarios finales y empresas a realizar cálculos externos y heterogéneos para sus reportes de sustentabilidad (ESG) y para el cumplimiento de obligaciones climáticas (Sistema de Comercio de Emisiones), restando competitividad y utilidad práctica al CEL frente a certificados internacionales (como I-REC) que ya integran esta métrica.

\subsection{Visión Objetivo (Situación Objetivo): Trazabilidad de Carbono e Integración ESG}
Se implementa el enriquecimiento del estándar de datos del CEL para incluir el Atributo de Carbono Referenciado.

Bajo este esquema, el SNIEr inyectará automáticamente en la metadata de cada certificado el Factor de Emisión del Sistema Eléctrico Nacional correspondiente al año y zona de gerencia de la generación. Esto convierte al CEL en un instrumento de ``Doble Propósito'':

\begin{itemize}
    \item \textbf{Regulatorio:} Acredita el cumplimiento de la obligación de consumo de energía limpia ante la CRE.
    \item \textbf{Climático:} Proporciona un dato oficial y auditado sobre las emisiones evitadas, facilitando el cálculo de Inventarios de Emisiones Alcance 2 (electricidad) para empresas con metas de descarbonización.
\end{itemize}

\subsection{Arquitectura de Sistemas (Metadata Extendida)}
El registro digital del CEL se expande para incluir campos informativos no transaccionales:

\begin{itemize}
    \item \textbf{Campo ``Factor de Red'':} Valor numérico (ej. 0.435 tCO$_2$e/MWh) tomado del Acuerdo anual publicado por la SEMARNAT/CRE.
    \item \textbf{Campo ``Emisiones Evitadas'':} Cálculo automático resultante (1 MWh $\times$ Factor).
    \item \textbf{Interoperabilidad ETS:} La estructura de datos se alinea con los requisitos del Registro Nacional de Emisiones (RENE) para facilitar futuras compensaciones cruzadas, si la regulación lo permite.
\end{itemize}

\subsection{Reingeniería de Procesos (Cálculo Automático)}

\begin{tabladoradoCorto}
  \caption{Tabla 16.1 Del Cálculo Manual al Reporte Integrado}
  \begin{tabularx}{\textwidth}{L{0.15\textwidth} L{0.40\textwidth} X}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Etapa} & \encabezadodorado{Modelo Anterior (Manual)} & \encabezadodorado{Nuevo Modelo (Integrado)} \\
    \midrule
    \textbf{Emisión} & El Generador recibe un CEL que dice ``1 MWh Solar''. & \textbf{Emisión Enriquecida:} El CEL nace con la etiqueta: ``1 MWh Solar = 0.435 tCO$_2$e evitadas''. \\
    \textbf{Reporte Corporativo} & La empresa compra CELs y luego contrata consultores para calcular su huella de carbono usando factores genéricos. & \textbf{Reporte Directo:} El SNIEr emite un ``Informe de Retiro de CELs'' que ya totaliza las toneladas de CO$_2$ evitadas con base oficial, listo para auditoría ESG. \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}

\subsection{Blindaje Normativo (Reforma a las DACG)}
Se propone la modificación del Numeral 12 de las Disposiciones Administrativas de Carácter General (RES/174/2016) para ampliar el contenido del título.

\begin{displayquote}
\textbf{Modificación al Numeral 12 de las Disposiciones (RES/174/2016):}

\textbf{Numeral 12. De las Características y Contenido de los Certificados.}

\textbf{IV.} Cada Certificado que emita el Sistema deberá contener, inmutados digitalmente, al menos los siguientes datos informativos:

...

\textbf{f)} El Factor de Emisión Eléctrico del Sistema Eléctrico Nacional vigente al momento de la generación, expresado en toneladas de dióxido de carbono equivalente por megawatt-hora (tCO$_2$e/MWh), conforme a la publicación oficial de la autoridad competente.

\textbf{g)} La cuantificación referencial de emisiones evitadas asociada a dicho Certificado.
\end{displayquote}

\subsection{Beneficios e Impacto}
\begin{itemize}
    \item \textbf{Valorización del Activo:} Al hacer explícito el beneficio climático, el CEL se vuelve más atractivo para compradores voluntarios internacionales que buscan mitigar su huella de carbono.
    \item \textbf{Estandarización Nacional:} Elimina la dispersión de criterios en el cálculo de emisiones evitadas, estableciendo una ``Verdad Única'' basada en datos oficiales del gobierno mexicano.
    \item \textbf{Preparación para Mercados de Carbono:} Sienta las bases técnicas para una futura vinculación (coupling) entre el mercado de CELs y el Sistema de Comercio de Emisiones (SCE).
\end{itemize}

\portadaseccion{17}{Mercado Secundario y Bancabilidad}{}
\section{Mercado Secundario y Bancabilidad}

\subsection{Diagnóstico (Situación Actual): Asimetría de Información y Falta de Referencias}
La estructura actual del mercado de CELs se caracteriza por una alta opacidad en la formación de precios. Al realizarse la mayoría de las transacciones mediante contratos bilaterales privados (Over-The-Counter) sin obligaciones de reporte económico al regulador, no existe una curva de precios pública ni índices de referencia confiables (Price Discovery).

Esta asimetría de información genera dos efectos nocivos:

\begin{itemize}
    \item \textbf{Barrera Financiera:} Las instituciones bancarias no pueden valorar con certeza los flujos futuros de ingresos por venta de CELs, por lo que excluyen estos activos de las garantías de financiamiento (falta de bancabilidad).
    \item \textbf{Ineficiencia:} Los pequeños generadores y consumidores negocian en desventaja frente a grandes jugadores al desconocer el valor real de mercado del activo.
\end{itemize}

\subsection{Visión Objetivo (Situación Objetivo): Certeza Financiera y Transparencia}
Se implementa un mecanismo de Transparencia Agregada que obliga al registro de los términos económicos de las transacciones sin vulnerar el secreto comercial.

\begin{itemize}
    \item \textbf{Registro Transaccional:} El SNIEr condicionará la transferencia de titularidad de los CELs a la captura del precio unitario pactado en la operación.
    \item \textbf{Índices de Referencia:} Con la información recabada, la autoridad publicará reportes mensuales de precios promedio, máximos y mínimos, segregados por tecnología y plazo (Spot vs. Contrato).
    \item \textbf{Bancabilidad:} La existencia de un índice oficial auditable permitirá a los desarrolladores presentar el flujo de CELs como una fuente de pago reconocida (``Bankable Revenue Stream'') en sus modelos financieros ante la banca de desarrollo y comercial.
\end{itemize}

\subsection{Arquitectura de Sistemas (Módulo de Mercado)}
El SNIEr integrará funciones de inteligencia de mercado:

\begin{itemize}
    \item \textbf{Input Obligatorio:} Al ejecutar una transferencia tipo ``Traspaso Bilateral'', el sistema desplegará un campo obligatorio: ``Monto de la Contraprestación (\$MXN/CEL)''.
    \item \textbf{Validación de Rango:} Algoritmos para detectar ``Outliers'' (ej. precios de \$0.01 o excesivos) que detonen alertas de auditoría para prevenir prácticas de simulación o lavado.
    \item \textbf{Tablero Público:} Un dashboard de acceso abierto que muestre la evolución histórica del precio del CEL (Index), similar a los tableros de precios del Mercado Eléctrico Mayorista.
\end{itemize}

\subsection{Reingeniería de Procesos (Formación de Precios)}

\begin{tabladoradoCorto}
  \caption{Tabla 17.1 De la Opacidad a la Transparencia de Mercado}
  \begin{tabularx}{\textwidth}{L{0.15\textwidth} L{0.40\textwidth} X}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Etapa} & \encabezadodorado{Modelo Anterior (Opaco)} & \encabezadodorado{Nuevo Modelo (Transparente)} \\
    \midrule
    \textbf{Negociación} & El comprador impone precio; el vendedor no tiene referencias para defender su oferta. & \textbf{Informado:} Ambas partes consultan el ``Precio Promedio Ponderado'' del mes anterior en el SNIEr antes de negociar. \\
    \textbf{Transferencia} & Se movían los CELs en sistema ``a ciegas'' (solo cantidad). & \textbf{Registro:} Se captura el valor económico de la transacción, creando una base de datos de inteligencia de mercado. \\
    \textbf{Financiamiento} & El banco valúa el CEL a \$0 por riesgo regulatorio. & \textbf{Valoración:} El banco utiliza el Índice Histórico SNIEr para proyectar ingresos y otorgar crédito. \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}

\subsection{Blindaje Normativo (Reforma a las DACG)}
Se propone modificar el Numeral 20 de las Disposiciones Administrativas de Carácter General (RES/174/2016) para vincular el registro operativo con el reporte económico.

\begin{displayquote}
\textbf{Modificación al Numeral 20 de las Disposiciones (RES/174/2016):}

\textbf{Numeral 20. De la Transferencia y Registro de Precios.}

\textbf{I.} Los Participantes podrán transferir libremente la titularidad de sus Certificados a través del Sistema. Para que dicha transferencia surta plenos efectos, el Participante cedente deberá ingresar la cantidad de Certificados objeto de la operación y el precio unitario pactado por cada uno de ellos.

\textbf{II.} La información de precios recabada tendrá el carácter de confidencial y será tratada bajo estrictos protocolos de seguridad. La Comisión utilizará dicha información exclusivamente para fines de vigilancia de mercado y para la elaboración de estadísticas agregadas.

\textbf{III.} La Comisión publicará mensualmente en el portal del Sistema un Índice de Precios de Certificados, que refleje el promedio ponderado de las transacciones registradas, con el fin de brindar certeza y transparencia a los Participantes del Mercado.
\end{displayquote}

\subsection{Beneficios e Impacto}
\begin{itemize}
    \item \textbf{Liquidez:} Facilita el encuentro entre oferta y demanda al reducir los costos de transacción por búsqueda de información.
    \item \textbf{Acceso a Capital:} Potencializa el apalancamiento de proyectos renovables al convertir al CEL en una garantía financiera real.
    \item \textbf{Señal de Inversión:} Un precio transparente envía la señal económica correcta sobre la escasez o abundancia de energía limpia, incentivando la inversión donde más se necesita.
\end{itemize}
