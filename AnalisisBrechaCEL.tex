...
\subsection{Diagnóstico Forense (As-Is): La ``Doble Venta'' del Atributo}

Bajo el marco anterior, no existía una vinculación jurídica ni tecnológica entre el sistema S-CEL (nacional) y los registros privados internacionales como I-REC (\textit{International Renewable Energy Certificate}) o bonos de carbono.

\begin{description}
    \item[La Falla:] El ``Divorcio de Registros''.
    \item[El Fraude:] Un generador eólico producía 1 MWh. Recibía 1 CEL en la plataforma de la CRE (y se lo vendía a CFE Suministro Básico). Simultáneamente, registraba ese mismo MWh en la plataforma I-REC y vendía el certificado ``verde'' a una multinacional (ej. Google o Bimbo) para sus metas de sustentabilidad.
    \item[Resultado:] El atributo ambiental (la ``limpieza'') se monetizaba dos veces. CFE decía ``yo consumí esa energía limpia'' y la multinacional también decía ``yo consumí esa energía limpia''. Físicamente imposible.
    \item[Consecuencia:] Descrédito internacional y riesgo de que los CELs mexicanos sean vetados en tratados comerciales (T-MEC) o taxonomías verdes europeas.
\end{description}

\subsection{Visión Objetivo (To-Be): Integridad ``Uno a Uno''}

El nuevo modelo SNIEr establece el principio de Unicidad del Atributo.

\begin{itemize}
    \item \textbf{Principio Rector:} 1 MWh Limpio = 1 Activo Digital. Ese activo puede llamarse CEL o puede canjearse por otro instrumento, pero no pueden existir ambos simultáneamente.
    \item \textbf{Mecanismo:} ``Quema Cruzada'' (\textit{Cross-Burning}). Si quieres emitir un I-REC, tienes que ``quemar'' (cancelar) el CEL correspondiente en el SNIEr para demostrar que no se usó para cumplimiento legal.
\end{itemize}

\subsection{Arquitectura de Sistemas (Especificación Técnica SNIEr)}

El SNIEr debe funcionar como la cámara de compensación central de atributos ambientales.

\subsubsection*{Serialización Universal}
El UUID del CEL incluye metadatos compatibles con el estándar EECS (\textit{European Energy Certificate System}) o I-REC Standard.

\subsubsection*{API de Intercambio (Bridge)}
\begin{itemize}
    \item Si un Generador solicita la emisión de I-RECs a un certificador autorizado (ej. ANCE), el sistema del certificador consulta al SNIEr vía API.
    \item \textbf{Lógica de Bloqueo:} SNIEr responde: ``Esos MWh ya tienen un CEL activo. Para emitir el I-REC, debo destruir el CEL \#12345''.
    \item \textbf{Confirmación:} El Generador acepta, el CEL muere en México, y nace el I-REC en el registro global.
\end{itemize}

\subsubsection*{Portal Voluntario}
Un módulo donde empresas no obligadas (Retail, Banca) pueden comprar y retirar CELs directamente para obtener un ``Certificado de Consumo Sustentable'' avalado por SENER, sin recurrir a mercados paralelos.
...
\begin{tabladoradoCorto}
  ...
  \begin{tabularx}{\textwidth}{L{0.20\textwidth} L{0.35\textwidth} X}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Variable} & \encabezadodorado{Modelo Anterior (Canibalismo)} & \encabezadodorado{Nuevo Modelo (Integridad)} \\
    \midrule
    \textbf{Emisión} & Generador pedía CELs a CRE y I-RECs a privados por separado. Nadie cruzaba los datos. & \textbf{Emisión Única:} El SNIEr es la fuente primaria. Todo nace como CEL. Si quieres otra cosa, debes convertirlo. \\
    \textbf{Cumplimiento} & Empresas presumían ``100\% Renovable'' con I-RECs baratos, mientras CFE cargaba con el costo de los CELs. & \textbf{Transparencia:} Si usas el atributo para marketing (Voluntario), ese atributo sale del pool de cumplimiento nacional (Obligatorio). \\
    \textbf{Certificación} & Confusa. Sellos privados sin respaldo gubernamental. & \textbf{Sello Oficial:} La CNE emite un diploma digital QR anual: ``Esta empresa retiró voluntariamente 5,000 CELs, equivalentes a evitar X toneladas de CO2''. \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}
...
Es imperativo prohibir explícitamente el doble beneficio en las Disposiciones.

\begin{displayquote}
\textbf{Artículo [M]. De la Unicidad del Atributo Ambiental y Prohibición de Doble Conteo.}
\begin{enumerate}[label=\Roman*.]
    \item Cada Certificado de Energía Limpia ampara la titularidad exclusiva de los beneficios ambientales asociados a la generación de un MWh de energía libre de combustibles.
    \item Queda estrictamente prohibida la emisión concurrente de Certificados y cualquier otro instrumento de acreditación ambiental, nacional o internacional, sobre el mismo volumen de energía generado.
    \item Los Generadores que deseen comercializar los atributos ambientales bajo estándares internacionales distintos al CEL, deberán solicitar ante el SNIEr la cancelación administrativa de los Certificados correspondientes, obteniendo una constancia de retiro que avale la unicidad del atributo frente a terceros certificadores.
\end{enumerate}
\end{displayquote}
...
\begin{itemize}
    \item \textbf{Credibilidad Internacional:} Los CELs mexicanos se vuelven ``Gold Standard'', aceptados plenamente en reportes globales como RE100 y CDP (Carbon Disclosure Project).
    \item \textbf{Valorización del CEL:} Al obligar al mercado voluntario (que es gigante) a comprar CELs reales (y quemarlos) en lugar de I-RECs paralelos, aumenta la demanda del CEL y sube su precio, beneficiando a los generadores.
    \item \textbf{Verdad al Consumidor:} Se protege al ciudadano de publicidad engañosa (``Greenwashing'').
\end{itemize}
...
\subsection{Visión Objetivo (To-Be): La Obligación Nace con el Electrón}

El nuevo modelo SNIEr cambia el principio jurídico de la obligación.

\begin{itemize}
    \item \textbf{Principio Rector:} La obligación de adquirir CELs nace en el momento físico del consumo, no en el momento administrativo del registro.
    \item \textbf{Mecanismo:} Cálculo Retroactivo Automático. El sistema no pregunta ``¿Cuándo te registraste?'', pregunta ``¿Cuándo prendiste la luz por primera vez?''.
\end{itemize}

\subsection{Arquitectura de Sistemas (Especificación Técnica SNIEr)}

El módulo de Cumplimiento integrará un Motor de Auditoría Forense.

\subsubsection*{Ingesta de Historia (CENACE History API)}
Al dar de alta un RFC, el SNIEr consulta al CENACE: ``Dame la Fecha de Inicio de Operación Comercial y el consumo mensual histórico de este Centro de Carga''.

\subsubsection*{Cálculo de Pasivos}
El algoritmo aplica la meta vigente en cada año pasado:
\begin{itemize}
    \item 2019: Consumo * 5.8\%
    \item 2020: Consumo * 7.4\%
    \item ...
\end{itemize}
\textbf{Output:} Genera un ``Estado de Cuenta Histórico'' con el saldo total adeudado.

\subsubsection*{Bloqueo de Operación}
El sistema impide realizar trámites nuevos (ej. cambios de suministrador) hasta que no se reconozca el adeudo histórico.

\subsection{Reingeniería de Procesos (Regularización)}

\begin{tabladoradoCorto}
  \caption{Tabla 13.1 Del Borrón y Cuenta Nueva a la Auditoría}
  \begin{tabularx}{\textwidth}{L{0.20\textwidth} L{0.35\textwidth} X}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Variable} & \encabezadodorado{Modelo Anterior (Borrón y Cuenta Nueva)} & \encabezadodorado{Nuevo Modelo (Cobro Retroactivo)} \\
    \midrule
    \textbf{Inscripción} & Usuario llenaba formato con fecha actual. & \textbf{Pre-llenado:} El sistema muestra: ``Detectamos actividad desde Mayo 2018''. \\
    \textbf{Detección} & Manual y aleatoria (si te tocaba visita de verificación). & \textbf{Sistemática:} Barrido automático mensual de todos los RFCs que facturan en CENACE vs los que pagan CELs. \\
    \textbf{Gestión de Deuda} & Inexistente. & \textbf{Esquema de Facilidades:} Se ofrece un plan de pagos (entregar los CELs atrasados en un plazo de 12-24 meses) a cambio de no imponer multas punitivas (``Amnistía Regulatoria''). \\
    \bottomrule
  \end{tabularx}
\end{tabladoradoCorto}
...
Es vital establecer que el registro es solo un acto formal, no el origen de la deuda.

\begin{displayquote}
\textbf{Artículo [N]. De la Exigibilidad y Cálculo Retroactivo.}
\begin{enumerate}[label=\Roman*.]
    \item La obligación de adquirir Certificados de Energías Limpias es exigible a partir del primer periodo de facturación en el que el Centro de Carga recibió suministro eléctrico, independientemente de la fecha de su inscripción en el Registro de Usuarios Calificados o en el SNIEr.
    \item Para los participantes que se inscriban de manera extemporánea, el SNIEr calculará automáticamente el Pasivo Histórico de Certificados acumulado desde el inicio de sus operaciones.
    \item La CNE podrá establecer Programas de Regularización que permitan a los participantes cubrir dicho Pasivo Histórico en parcialidades, condonando las sanciones económicas siempre y cuando se garantice la entrega total de los Certificados adeudados.
\end{enumerate}
\end{displayquote}
...
\begin{itemize}
    \item \textbf{Recuperación de Demanda:} Se estima que existe una ``demanda oculta'' de 3 a 5 millones de CELs correspondientes a años anteriores no cobrados. Esto ayuda a limpiar la sobreoferta del mercado.
    \item \textbf{Equidad de Mercado:} Los usuarios cumplidos dejan de sentir que son los únicos que pagan la fiesta. Se nivela el terreno de juego.
    \item \textbf{Cultura de Cumplimiento:} El mensaje al mercado es claro: ``No puedes esconderte; el sistema tiene memoria''.
\end{itemize}
